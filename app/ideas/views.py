...
from . import ideas
from app.services import get_category_by_id, delete_category, create_category, list_categories, list_ideas_by_username, create_idea, delete_idea_db, update_idea_db, update_state_idea_db, get_idea_by_id
from .form import DeleteCategoryForm, RegisterCategoryForm, IdeaForm, DeleteIdeaForm, PublicIdeaForm
from app.utils import get_dict_from_wftorm
...
def contextHome():
    ...
    context = {
        'username': username,
        'idea': list_ideas_by_username(username),
        'idea_form': idea_form,
...
def delete_idea(idea_id):
    ...
    flash("Idea eliminada con exito", category="success")
    return redirect(url_for("ideas.home"))

@ideas.route("/public_idea/<idea_id>/<int:is_public", mehtods = ["POST"])
@login_required
def public_idea(idea_id, is_public):
    update_state_idea_db(idea_id,is_public)
    flash("Idea actualizada con éxito", category="Success")
    return redirect(url_for("ideas.home"))
...
def update_idea(idea_id):
    ...
    idea_form.title = idea.title
    idea_form.description = idea.description
    idea_form.is_public = idea.is_public
    idea_form.category = idea.category_id
    ...
    return render('ideas/home.hmtl', **context)
